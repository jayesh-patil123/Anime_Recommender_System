...
logger = setup_logger('HelpersLogger')
...
def getAnimeFrame(anime, path_df):
    try:
        ...
        logging.error(f"Error occurred while loading anime data: {e}")
        raise
...
def getSynopsis(anime, path_synopsis_df):
    try:
        ...
        logging.error(f"Error occurred while loading synopsis data: {e}")
        raise
...
def find_similar_animes(name, path_anime_weights, path_anime2anime_encoded, path_anime2anime_decoded, path_anime_df, n=10, return_dist=False, neg=False):
    try:
        ...
        logging.error(f"Error in find_similar_animes: {e}")
        return pd.DataFrame()
...
def find_similar_users(item_input, path_user_weights, path_user2user_encoded, path_user2user_decoded, n=10, return_dist=False, neg=False):
    try:
        ...
        logging.error(f"Error in find_similar_users: {e}")
        return pd.DataFrame()
...
def get_user_preferences(user_id, path_rating_df, path_anime_df):
    try:
        ...
        logging.error(f"Error in get_user_preferences: {e}")
        return pd.DataFrame()
...
def get_user_recommendations(similar_users, user_pref, path_anime_df, path_synopsis_df, path_rating_df, n=10):
    try:
        ...
        logging.error(f"Error in get_user_recommendations: {e}")
        return pd.DataFrame()


######## 7. GET RANDOM ANIME ########

def get_random_anime(n=10):
    try:
        df = pd.read_csv(path_anime_df)
        anime_list = df['eng_version'].dropna().unique().tolist()
        return list(np.random.choice(anime_list, size=n, replace=False))
    except Exception as e:
        logging.error(f"Error in get_random_anime: {e}")
        return []
